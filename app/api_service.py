...
class APIService:
    ...
    async def _handle_request(self, request: ContextRequest) -> ContextResponse:
        ...
        try:
            ...
            # 2) 短期メモリに埋め込み
            self.memory_manager.hierarchical_caching(request.session_id, self._to_embedding(inputs))
            ...
            symbolic_valid = self.system.ns_interface.symbolic_check(request.text, scope=request.session_id)
            final_consistency = (consistency_score < self.system.config.consistency_threshold) and symbolic_valid
